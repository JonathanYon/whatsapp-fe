...
function SideBar() {
  ...
  const addMembers = async () => {
    try {
      const response = await fetch(`https://what-s-app.herokuapp.com/chats`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${window.localStorage.getItem("Token")}`,
        },
        body: JSON.stringify({ members: newChatRoomMember }),
      });
      if (response.ok) {
  ...
  return (
    <Col id="sidebar">
      ...
      <ContactSearch />
      <ContactBoard />
      {newChatRoomMember.length > 0 && (
        <div id="addChatButton" onClick={addMembers}>
          <IoIosAddCircle />
        </div>
      )}
    </Col>
